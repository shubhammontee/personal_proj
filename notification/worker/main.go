...
func main() {
	...
	flag.StringVar(&kafkaTopic, "kafka-topic", "notification", "Kafka topic. Only one topic per worker.")
	...
	for {
		...
		_, restErr := http.Post("http://localhost:8080/offer", "application/json; charset=utf-8", bytes.NewBuffer(value))
		if restErr != nil {
			fmt.Println(restErr)
		}
